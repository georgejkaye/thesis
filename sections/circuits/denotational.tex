...
\begin{lemma}\label{lem:finitely-specified-preserved}
    Being finitely specified is preserved by composition and tensor product.
\end{lemma}
\begin{proof}
    For finitely many stream derivatives, both the composition and product of
    two stream functions \(f\) and \(g\), the largest the set of stream
    derivatives could be is the product of stream derivatives of \(f\) and
    \(g\), so this will also be finite.
\end{proof}
