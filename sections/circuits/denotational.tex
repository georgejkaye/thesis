...
\begin{proposition}[\protect\footnotemark]
...
\begin{proof}
    ...
    \(\widehat{f_\epsilon}\left((-)\sigma\right)\).
    ...
    \begin{align*}
        \streamderivative{\sigma \mapsto \left(\widehat{\streamderivative{f}{\, \listvar{w_k}}}(\left(g_{k-1}(\tau\sigma)\right)\sigma)\right)}{\listvar{ab}}
            &= \sigma \mapsto \streamderv\left(\widehat{\streamderivative{f}{\, \listvar{w_k}}}(\listvar{ab} \streamcons \left(g_{k-1}(\sigma)\right)\sigma)\right) \\
            &= \sigma \mapsto \streamderv\left(\proj{0}\left(\streamderivative{f}{\, \listvar{w_k}}(\listvar{ab} \streamcons \left(g_{k-1}(\sigma)\right)\sigma)\right)\right) \\
            &= \sigma \mapsto \proj{0}\left(\streamderv\left(\streamderivative{f}{\, \listvar{w_k}}(\listvar{ab} \streamcons \left(g_{k-1}(\sigma)\right)\sigma)\right)\right) \\
            &= \sigma \mapsto \proj{0}\left(\streamderv\left(\streamderivative{f}{\, \listvar{w_k}}(\listvar{ab} \streamcons \left(g_{k-1}(\sigma)\right)\sigma)\right)\right)
            % &= \streamderv\left(\proj{0}\left(f_{\, \listvar{w_k}}(ab \streamcons \left(g_{k-1}(\tau\sigma)\right)\sigma)\right)\right) \\
            % &= \proj{0}\left(\streamderv\left(f_{\, \listvar{w_k}}(ab \streamcons \left(g_{k-1}(\tau\sigma)\right)\sigma)\right)\right) \\
            % &= \proj{0}\left(\streamderivative{\left(f_{\, \listvar{w_k}}(\left(g_{k-1}(\tau\sigma)\right)\sigma)\right)}{ab}\right) \\
            % &= \proj{0}\left(\streamderivative{\left(f_{\, \listvar{w_k}}(\left(g_{k-1}(\tau\sigma)\right)\sigma)\right)}{ab}\right)
        % \streamderivative{(\widehat{\streamderivative{f}{\listvar{w}}})}{ab}(\tau\sigma)
        %     &= \streamderv\left(\widehat{\streamderivative{f}{\listvar{w}}}((ab \streamcons \tau\sigma))\right) \\
        %     &= \streamderv\left(\proj{0}\left(\streamderivative{f}{\listvar{w}}(ab \streamcons \tau\sigma)\right)\right) \\
        %     &= \proj{0}\left(\streamderv\left(\streamderivative{f}{\listvar{w}}(ab \streamcons \tau\sigma)\right)\right) \\
        %     &= \proj{0}\left(\streamderivative{\streamderivative{f}{\listvar{w}}(\tau\sigma)}{ab}\right) \\
        %     &= \proj{0}\left(\streamderivative{f}{ab \streamcons \listvar{w}}(\tau\sigma)\right) \\
        %     &= \widehat{\streamderivative{f}{ab \streamcons \listvar{w}}}(\tau\sigma)
    \end{align*}
    ...
    is a stream derivative of \(f\) and \(f\) has
    This means that \(\sigma \mapsto \proj{1}(f(\mu_f(\sigma)\sigma))\) lands
    in \(\streami\), so it is a valid trace.
\end{proof}
\footnotetext{The original proof idea comes from David Sprunger.}