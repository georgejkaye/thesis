...
\begin{corollary}
    ...
    =
    \circuittostreami[\iltikzfig{circuits/productivity/mealy-form}[core=g]]
    \).
\end{corollary}

By \cref{thm:possible-encodings}, we have that there is an encoding that maps
states between denotationally equivalent circuits.
If we can encode a circuit's state to one comprised of only \(\bot\) and
\(\top\) elements, does this mean we have a sound and complete equational
theory?
Unfortunately not: all it means is that the circuits agree on the set of
circuit states.

\begin{example}
    \todo[inline]{Do example to establish}
\end{example}
