...
To illustrate why this is the case, we first provide a notion for computing the
states a syntactic circuit can assume.
...
Crucially, two circuits with the same semantics might have a different number of
states, just like two bisimilar Mealy machines.
The optimistic reader might think that we we might still be able to translate
one circuit into the other through some combination of using the
normalisation equations and sliding components around the trace.
Unfortunately, these may be insufficient when the circuits have
a different number of states.

\todo[inline]{Example? not iso on all states}
...
\begin{lemma}
    Given Mealy homomorphisms \(\morph{h}{(S,f)}{(T,g)}\) and
...
\begin{lemma}\label{lem:bisimulation-initial-states}
