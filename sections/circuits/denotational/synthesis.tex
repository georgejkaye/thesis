...
\begin{remark}
    ...
    Note also that this sequential component is by no means mandatory: it could
    be of width \(0\) and the functional completeness map could only map to
    truly combinational circuits.
\end{remark}
...
\begin{proof}
    \(\circuittomealyi[
        \iltikzfig{circuits/productivity/mealy-form}[core=\hat{f},state=\listvar{s},dom=m,cod=n,delay=x]
    ]\) is the fixpoint of the Mealy machine \[
        \left(\valuetuple{x},
        (\listvar{r}, \listvar{v}) \mapsto \left\langle
        \listvar(r), \hat{f}(\listvar{s}, \listvar{v})
        \right\rangle, \listvar{s}\right)
    \], which immediately stabilises to \[
        \left(\valuetuple{x},
        \listvar{v} \mapsto \left\langle
        \proj{0}\left(\hat{f}(\listvar{s},\listvar{v})\right),
        \proj{1}\left(\hat{f}(\listvar{s}, \listvar{v})\right)
        \right\rangle, \listvar{s}\right)
    \]
...
\end{theorem}
\begin{proof}
    \todo[inline]{Update for Mealy machine trip}
