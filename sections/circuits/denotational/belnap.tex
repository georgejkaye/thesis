\section{Denotational semantics for Belnap logic}
...
These should truth tables should look familiar; indeed, if one squints a little
...
Since an expression can be defining using only operations with counterparts in
the syntactic realm, it is possible to define a map \(
\morph{\mealytofunc_\belnap}{\func{\interpretation_\belnap}}{\scirc{\Sigma_\belnap}}
\) such that \(\mealytofunc[f]_\belnap\) is a term of the form \(
