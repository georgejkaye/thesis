...
If there is no pushout complement, there is no possible rewrite, so when
performing graph rewriting it is essential to know when one exists.
...
\begin{example}\label{ex:non-left-linear}
    ...
    \]
    This matching yields the following valid pushout complements and subsequent
    rewrites:
    \begin{gather*}
...
\begin{example}
    ...
    \end{gather*}
    All of the rewrites other than the first were `hidden' behind the equations
    of a Frobenius algebra, but arose naturally in the graph rewriting
    framework.
\end{example}
