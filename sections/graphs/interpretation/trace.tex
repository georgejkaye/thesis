...
\subsection{Monogamy and acyclicity}
...
\end{lemma}
\begin{proof}
    For (1), all cospans involved are discrete and all vertices are in both
    interfaces; there are clearly no cycles.
    ...
    Moreover, a cycle cannot be created because there cannot be a path in \(F\)
    that starts in the image of \(g\) or a path in \(G\) that ends in the image
    of \(h\), because these vertices have out-degree and in-degree \(0\)
    respectively.
    ...
    Cycles cannot be created because the two graphs do not interact with each
    other.
\end{proof}
