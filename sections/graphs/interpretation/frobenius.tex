...
cocommutative comonoids along with the `Frobenius' and the 'special' equation.
...
\begin{theorem}[\cite{bonchi2022string}, Thm. 3.8]
    ...
    let \(\mathcal{C}\) be a category such that \(\mathbb{X}\) and
    \(\mathcal{C}\) have finite limits, and let
    \(\morph{F}{\mathbb{X}}{\mathbf{C}}\) be a colimit-preserving functor.
...
It is now possible to map from coloured Frobenius terms to cospans of coloured
hypergraphs.
