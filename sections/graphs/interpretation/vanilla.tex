...
created.
The former is tackled by a condition on the connectivity of nodes.

\begin{definition}[Degree {\cite[Def.\ 12]{bonchi2022stringa}}]
    For a hypergraph \(F \in \hyp\), the \emph{degree} of a node
...
\end{lemma}
\begin{proof}
    The cospans involved are discrete and all nodes are in both
...
\begin{proof}
    ...
    \(\hypsigmac\).
    The only altered nodes are those in the image of
    \(g\) and \(h\), which are merged pointwise; nodes in the image of
    \(g\) have out-degree \(0\) and those in the image of \(h\) have in-degree
    \(0\) so the merged nodes have at most degree \((1, 1)\).
\end{proof}
...
\begin{proof}
    ...
    only nodes in the original interfaces will be in the new interfaces.
\end{proof}
...
\begin{proof}
    ...
    image of \(h\), because these nodes have out-degree and in-degree \(0\)
    respectively.
\end{proof}
