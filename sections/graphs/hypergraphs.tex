...
\subsection{Frobenius structure and hypergraphs}
...
We can now move towards showing an isomorphism \(
    \cfrob{C} \cong \csp{\finsetpropwithnat \slice C}
\) for countable \(C\).
...
\begin{definition}[\cite{bonchi2022string}]
    ...
    \(\morph{D}{\finsetpropwithnat \slice \mcs}{\hypsigma}\) be the faithful,
    coproduct-preserving functor that sends each word
    \(\listvar{x} \in \finsetpropwithnat\) to the discrete hypergraph
