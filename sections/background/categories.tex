...
\begin{example}[Monoid]
    A \emph{monoid} is a tuple \((X, *, \varepsilon)\) where \(X\) is a set,
    \(*\) is an associative operation \(X \times X \to X\), and
    \(\varepsilon \in X\) is a \emph{unit}, i.e.
    \(\varepsilon * x = x = x * \varepsilon\).
    Any monoid generates a category with a single object \(X\); the morphisms
    \(X \to X\) are the (partially applied) functions \(- * x\) for each
    \(x \in X\).
    The identity morphism is \(- * \varepsilon\).
\end{example}
...
Monoidal theories can be used to reason with many structures in mathematics; we
will now give such an example.
