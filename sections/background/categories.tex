...
interpretation, which will make understanding terms and equations much easier.
...
\begin{lemma}\label{lem:mc-equations}
    The following equations hold in any strict monoidal category, given morphisms
    \(f\), \(g\), \(h\) and \(k\):
    \begin{gather*}
        f \tensor \id[I] = f
        \quad
...
\begin{lemma}\label{lem:smc-equations}
    The following equations all hold in a strict symmetric monoidal category:
    \begin{gather*}
        \swap{A}{I} = \id[A]
        \quad
        \swap{I}{A} = \id[A]
...
\(\generators\)-terms are now subject to the equations introduced in
\cref{def:category}, \cref{lem:mc-equations}, and \cref{lem:smc-equations}.
