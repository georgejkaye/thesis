...
\begin{notation}[Lists]
    ...
    \(\freemon{X}\).
    Arbitrary lists are written with an overline \(
        \overline{x}, \overline{y}, \overline{z}... \in \freemon{X}
    \); given two lists \(\overline{x}, \overline{y}\), their concatenation is
    denoted \(\overline{x} \concat \overline{y}\).
\end{notation}
...
Now we have some categorical knowledge, it is easy to see how processes can be
viewed as morphisms: a morphism \(\morph{f}{A}{B}\) is a process that transforms
some input of type \(A\) into an output of type \(B\).
...
\begin{definition}[Multi-sorted PROP~\cite{maclane1965categorical}]
    A multi-sorted \emph{PROP} (category of \emph{PRO}ducts and
    \emph{P}ermutations) is a symmetric monoidal category with the objects as
    lists of natural numbers and tensor product as list concatenation.
\end{definition}

Note that this definition means that the empty list \([]\) is the unit object in
any PROP.
A morphism \(\overline{m} \to \overline{n}\) in a PROP can be thought of as a
process with \(\overline{m}\) inputs and \(\overline{n}\) outputs.
