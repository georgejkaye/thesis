...
\).
The special case of the identity on \(\epsilon\) is usually drawn as empty space
\(
    \iltikzfig{strings/monoidal/empty}
\); when it is necessary to indicate that a unit is being used in an equation
then it will be drawn as a faded wire \(
    \iltikzfig{strings/monoidal/unit-wire}[white][\epsilon]
\).
...
Of course, this just syntax; rather than writing symbols we are drawing boxes
instead.
