...
When working with digital circuits, the first thing to do is to construct them.
Categorically, this corresponds to defining a category in which a morphism
\(m \to n\) is a circuit with \(m\) inputs and \(n\) outputs.
These circuits are purely \emph{syntactic}, in that they do not have meaning
assigned to them other than their structure.
...
\begin{example}[More logic gates]
    ...
    \) acts as as the inverse of an \(\andgate\) gate: it
...
disconnected \(\bullet\) vaulue after that.
...
\begin{example}[SR latch]\label{ex:sr-latch}
    ...
    It is only until the Reset input is pulsed true that the \(\mathsf{Q}\)
