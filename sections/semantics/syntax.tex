...
\begin{example}
    ...
    \), where \begin{gather*}
        ...
        \belnaparity^+
        \coloneqq
        \andgate_{k,n} \mapsto [n \,|\, i < k],
        \orgate_{k,n} \mapsto [n \,|\, i < k],
        \notgate_{n} \mapsto [n],
        \mathsf{add}_n \mapsto [n,n]
        \\
        \belnapcoarity^+
        \coloneqq
        \andgate_n \mapsto [n],
...
The first row of generators in \(\ccircsigmaplus\) is fairly straightforward:
primitives with appropriately typed wires and wire constructs for each
colour (width) of wires.

The generators specified on the bottom row are known as \emph{bundlers}: their
intended meaning is that they can be used to \emph{split} and \emph{combine}
bundles of wire buses into bundles with different widths.
