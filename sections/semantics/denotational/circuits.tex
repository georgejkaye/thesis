...
order operation; for example, in the above definitions we have used \(\wedge\)
and \(\vee\) for the order \(\leq\), and in subsequent sections we will use
\(\lmeet\) and \(\ljoin\) for the order \(\sqsubseteq\).
...
\begin{proof}
    ...
    can define the element \(x\) as the meet of every
    element in \(A\), i.e.\ \(
    x \coloneqq (((a_0 \wedge a_1) \wedge a_2) \wedge \dots a_{n-1})
    \).
    ...
    The same proof holds in reverse for the join and the greatest element.
\end{proof}
