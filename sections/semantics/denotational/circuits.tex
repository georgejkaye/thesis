...
Since the components of our circuits are monotone, a denotational semantics for
circuits must take values in stream functions which are themselves monotone.
...
\begin{lemma}\label{lem:finite-monotone-scott}
    In a \((A, \leq)\) in which every chain \(a_0 \leq a_1 \leq \cdots a_{n-1}\)
    is finite, all monotone functions \(A \to A\) are Scott-continuous.
\end{lemma}
\begin{proof}
    The directed subsets of a lattice with finite chains are the subsets with a
    greatest element; for such a subset \(B\) with greatest element \(b_0\) we
    have that \(\bigvee B = b_0\).
    Moreover, for a function \(\morph{f}{A}{A}\) and \(b \in B\) we have that
    \(f(b) \leq f(b_0)\), so \(f(b_0)\) is the greatest element in \(f[B]\).
    Subsequently, \(f(\bigvee B) = f(b_0) = \bigvee f[B]\) and so the function
    is Scott-continuous.
\end{proof}
