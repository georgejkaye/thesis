...
\begin{definition}[Minimal and maximal elements]
    In a poset \((A, \leq)\), a \emph{minimal element} is an element \(b \in A\) such that for
    all elements \(y \in A\), \(b \leq y\).
    Similarly, a \emph{maximal element} is an element \(a \in A\)
    such that for all elements \(x \in A\), \(x \leq a\).
\end{definition}

\begin{example}
    The natural numbers \(\nat\) are a poset ordered in the usual way; they have
    a minimal element \(0\) but not a maximal element.
    However, any finite subset of the natural numbers \emph{does} have a
    maximal element.
\end{example}

The above example illustrates a useful property induced by the \emph{finiteness}
of a poset.

\begin{lemma}\label{lem:maximal-minimal}
    In a finite poset \((A, \leq)\), there exists at least one maximal element
    and at least one minimal element.
\end{lemma}
\begin{proof}
    If there is no maximal element, then \(A\) has an infinite chain of
    elements \(a_1 \leq a_2 \leq a_3 \leq \cdots\), which contradicts the
    finiteness of \(A\).
    The same proof holds for the minimal element.
\end{proof}

Even in a finite set there is no reason that minimal and maximal elements
are \emph{unique}.
For our values we would like this to be the case: the unique minimal element
will represent a complete \emph{lack} of information, and the unique maximal
element will represent \emph{every} piece of information at once.
To enforce the uniqueness of these elements we must add even more structure to
a poset of values, elevating it to what is known as a \emph{lattice}.
...
\begin{lemma}
    If a poset \((A, \leq)\) has an infimum then it is unique, and if it has a
    supremum then it is also unique.
\end{lemma}
\begin{proof}
    Assume that \(A\) has two infima \(a\) and \(b\).
    This means that both \(a\) and \(b\) are lower bounds, and as \(a\) is a
    \emph{greatest} lower bound then \(b \leq a\).
    But \(b\) is also a greatest lower bound, so \(a \leq b\).
    Therefore \(a = b\).
    The same principle holds for the uniqueness of the supremum.
\end{proof}
...
\begin{definition}[Lattice]
    ...
    \emph{bounded} if \(A\) has an infimum and a supremum.
\end{definition}
...
\end{lemma}
\begin{proof}
    We know by \cref{lem:maximal-minimal} that \(A\) must have at least one
    minimal and maximal elements as it is finite.
\end{proof}


In a \emph{finite} lattice there must exist an element called the
\emph{supremum} (denoted\(\top\))
...
\begin{proposition}
    The function \(\sigma \mapsto \proj{1}\mleft(f(\mu_f(\sigma)\sigma)\mright)\)
