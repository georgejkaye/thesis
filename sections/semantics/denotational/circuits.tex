...
We are interested in the `closest' lower and upper bounds.
...
\begin{definition}[Lattice]
    ...
    \emph{bounded} if it has a least element and a greatest element.
\end{definition}

Much like how every non-empty finite poset has at least one least and
greatest element, every non-empty finite lattice has a join and meet.
...
\begin{proof}
    ...
    can define the least element \(\bot\) as \(\bigwedge A\).
    This element is the least element of \(A\) by definition of the meet:
    ...
    The proof holds in reverse for the greatest element \(\top\) by using the
    join \(\bigvee A\).
\end{proof}
...
lattice, so that combining two signals into one wire can be modelled using the
join.
...
\begin{definition}[Interpretation]
    ...
    \nomenclature{\(\ljoin\)}{join in a lattice of circuit values \(\values\)}
    \nomenclature{\(\sqsubseteq\)}{ordering in a lattice of circuit values \(\values\)}
    For a signature \(
    \signature = (
...
\begin{definition}
    \nomenclature{\(\circuittofunci\)}{PROP morphism \(\ccircsigma \to \funci\)}
