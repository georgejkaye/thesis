...
possible values the generator can take.
In possibility \(0\) the value will be \(\listvar{v?}(0)\), in possibility \(1\)
the value will be \(\listvar{v?}(1)\), and so on up to \(\listvar{v?}(k-1)\).
If an uncertain value word has length shorter than the number of possibilities,
we take the convention that the value is interpreted as \(\bot\).
